module ProfilesHelper
  ...
  def show_map
    @map = GMap.new("map_div")
    if @event
      marker = create_marker(@event.marker, @event.title, 'event', '/assets/yellow_star.png')
    elsif(@profile.marker)
      marker = create_marker(@profile.marker, @profile.full_name({:is_short => 1}), 'user', '/assets/yellow-dot.png')
  ...
  def create_marker(marker, title, image = 'marker', image_url = '/assets/marker.png')
    info = marker.profile ? set_icon(marker.profile, 'small_60') : '/assets/event_star.jpeg'
